...
GameEngine.prototype.onTimerTick = function(){
	var now = Date.now();
	this.game.update(now - this.lastUpdate);
	this.lastUpdate = now;
	this.drawScreen();
}
...
GameEngine.prototype.keyDownListener = function(evt) {
	this.game.keyDownListener(evt.keyCode);
}
GameEngine.prototype.keyUpListener = function(evt) {
	this.game.keyUpListener(evt.keyCode);
}
